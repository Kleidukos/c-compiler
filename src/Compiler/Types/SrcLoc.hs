...
mkSrcLoc name line col = RealSrcLoc $ SrcLoc (T.pack name) line col

mkRealSrcLoc :: String -> Int -> Int -> RealSrcLoc
mkRealSrcLoc = SrcLoc . T.pack
...
mkUnhelpfulSrcLoc = UnhelpfulLoc . T.pack
...
mkUnhelpfulSrcSpan = UnhelpfulSpan . T.pack
...
spans span (l, c) = realSrcSpanStart span <= loc && loc <= realSrcSpanEnd span
  where
    loc = SrcLoc (srcSpanFile span) l c
